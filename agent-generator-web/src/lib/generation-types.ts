...
export const generateRequestSchema = z.object({
  ...
  /** OpenAI chat model (e.g. gpt-4o-mini). Defaults to OPENAI_MODEL / server default. */
  openaiModel: z.string().regex(/^[A-Za-z0-9._:-]{2,80}$/).optional(),
  /** Extra instructions for skills / intents (also used for org field-hint extraction). */
  intentResearchInstructions: z.string().optional(),
  /** Do not generate intentDeployPlan rows */
  skipIntents: z.boolean().optional(),
  /** Handler + prompt commands only (minimal agent prompt placeholders) */
...
export const generatedBundleSchema = z.object({
  ...
  /** Optional sample user queries for QA and intent coverage checks. */

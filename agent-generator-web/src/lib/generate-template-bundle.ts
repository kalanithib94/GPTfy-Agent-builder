 ...
 */
public with sharing class ${handlerClass} implements ${agenticInterface} {
...
export function buildTemplateBundle(
  ...
  useCase: string,
  notes?: string,
  options?: { gptfyNamespace?: string }
): GeneratedBundle {
  const agenticInterface = resolveAgenticInterfaceSymbol(options?.gptfyNamespace);
  const logPrefix = params.handlerClass.replace(/Handler$/, "").substring(0, 8).toUpperCase();
  const handlerApex = buildHandlerApex(params.handlerClass, logPrefix, agenticInterface);
